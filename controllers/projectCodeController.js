...
const getAllProjectCodes = asyncHandler(async (req, res) => {
  ...
  if (!projectCodes?.length)
    return res
      .status(400)
      .json({ message: "BAD REQUEST : No project codes found" });

  const projectCodesWithUser = await Promise.all(
    projectCodes.map(async (projectCode) => {
...
const createNewProjectCode = asyncHandler(async (req, res) => {
  const { userId, media, year, finalCustomerId, brand, desc, month, code } =
    req.body;
  ...
  try {
    ...
    res.status(400).json({
      message: "BAD REQUEST: Unable to create project code",
      error: error.message,
    });
  }
});
...
const updateProjectCode = asyncHandler(async (req, res) => {
  const { id, userId, media, year, finalCustomerId, brand, desc, month } =
    req.body;
...
const deleteProjectCode = asyncHandler(async (req, res) => {
  const { id } = req.body;
  if (!id) return res.status(400).json({ message: "Project Code ID required" });
