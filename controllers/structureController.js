...
const {
  updateExpiredStructures,
} = require("../middleware/updateExpiredStructures");
...
const getAllStructures = asyncHandler(async (req, res) => {
  ...
  // const structures = await Structure.find({ name: { $regex: /\d$/ } }).lean();
  const structures = await Structure.find().lean();
  if (!structures?.length)
    return res
      .status(400)
      .json({ message: "BAD REQUEST : No structures found" });

  const structuresWithUser = await Promise.all(
...
const createNewStructure = asyncHandler(async (req, res) => {
  ...
  const duplicate =
    name.startsWith("N") || name.startsWith("B")
      ? await Structure.findOne({ name }).lean().exec()
      : null;
  if (duplicate)
    return res
      .status(409)
  ...
  if (structure)
    return res.status(201).json({
      message: `CREATED: Structure ${req.body.name} created successfully!`,
    });
  else
    return res
      .status(400)
...
const updateStructure = asyncHandler(async (req, res) => {
  const { id, userId, name, location, isAvailable, isChosen, parent } =
    req.body;
  ...
  if (!structure)
    res.status(400).json({ message: "BAD REQUEST : Structure not found" });
...
const deleteStructure = asyncHandler(async (req, res) => {
  const { id } = req.body;
  if (!id) return res.status(400).json({ message: "Structure ID required" });
